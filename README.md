...
tart의 이미지 용량 확장은 cli로는 어렵습니다.
vanilla이미지를 클론받았다면, 기본 용량이 xcode를 설치하기에는 모자랄 수 있습니다.
자세한 가이드는 [여기](https://tart.run/faq/#disk-resizing)를 확인하여 주세요 

1. tart run <vm-name> --recovery
2. Options 선택 후 Terminal 실행
3. disk 상태 확인 `diskutil list physical`
4. recovery partition이 있다면 제거 `diskutil eraseVolume free free disk0s3`
5. disk 복구 `yes | diskutil repairDisk disk0`
6. disk 확장 `diskutil apfs resizeContainer disk0s2 0`
...
xcodes download $TARGET_XCODE_VERSION --directory $CACHE_DIR

BASE_VM="tahoe-base"
VM_NAME="${BASE_VM}-xcode-${TARGET_XCODE_VERSION}"
DISK_SIZE="80"
...
tart run ${VM_NAME} --recovery

```

vm에 용량이 충분하다면, 위에서 준비한 이미지에 xcode를 설치합니다.
...
PKR_VAR_xcodes_username="$APPLE_USER_ID" \
PKR_VAR_xcodes_password="$APPLE_USER_PASSWORD" \
